...
class EquipmentPersistenceTests(unittest.TestCase):
    def test_existing_fine_ocr_record_is_written_and_is_idempotent(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            stats = database.connection.execute("SELECT * FROM equipment_stats WHERE item_id=?", (record["item_id"],)).fetchall()
            ...
            self.assertEqual(len(stats), 1)  # Attack main stat; unknown life/defense remain in raw JSON.
    ...
    def test_dictionary_hook_is_defined_but_not_used_by_default(self):
        record = {"item_id": "x", "profile": "general", "fully_unlocked": True,
                  "quality": {"raw_text": "装备"}, "slot": {"raw_text": "武器"},
                  "primary": {"raw_text": "攻击 10", "value": 10}, "set_name": {"raw_text": "套装"},
                  "sub_attributes": []}
        item, stats, _ = build_database_rows(record)
        self.assertEqual(item[1], "weapon")
        self.assertEqual(stats[0][2], "ATK_FLAT")

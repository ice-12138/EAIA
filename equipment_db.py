...
def _record_stat_type(field: object) -> str | None:
    raw = str(field.get("raw_text", "") if isinstance(field, dict) else field).upper()
    if "暴击率" in raw or "CRIT_RATE" in raw:
        return "CRIT_RATE"
    if "暴击伤害" in raw or "CRIT_DMG" in raw:
        return "CRIT_DMG"
    if "攻速" in raw or "攻击速度" in raw or "ATK_SPEED" in raw:
        return "ATK_SPEED"
    if "攻击" in raw and ("%" in raw or "加成" in raw or "ATK_PCT" in raw):
        return "ATK_PCT"
    if "攻击" in raw or "ATK_FLAT" in raw:
        return "ATK_FLAT"
    if "怒气" in raw or "能量回复" in raw or "RAGE_REGEN" in raw:
        return "RAGE_REGEN"
    return None
...
CREATE TABLE IF NOT EXISTS heroes (
 hero_id TEXT PRIMARY KEY, hero_name TEXT NOT NULL, atk_base REAL NOT NULL CHECK(atk_base >= 0),
 crit_rate_base REAL NOT NULL CHECK(crit_rate_base BETWEEN 0 AND 1), crit_dmg_base REAL NOT NULL CHECK(crit_dmg_base >= 1),
 atk_speed_base REAL NOT NULL, atk_interval_base REAL, rage_start REAL NOT NULL DEFAULT 0, rage_max REAL NOT NULL DEFAULT 0,
 damage_type TEXT NOT NULL CHECK(damage_type IN ('physical','magic','true')), main_output TEXT NOT NULL CHECK(main_output IN ('single','aoe','mixed'))
);
CREATE TABLE IF NOT EXISTS skills (
 hero_id TEXT NOT NULL REFERENCES heroes(hero_id), skill_id TEXT NOT NULL, skill_name TEXT NOT NULL,
 source_type TEXT NOT NULL CHECK(source_type IN ('basic','skill','ultimate','followup')), scaling_stat TEXT NOT NULL,
 coefficient REAL NOT NULL CHECK(coefficient >= 0), hit_count INTEGER NOT NULL CHECK(hit_count > 0), target_cap TEXT NOT NULL,
 can_crit INTEGER NOT NULL CHECK(can_crit IN (0,1)), cooldown REAL, action_time REAL, rage_cost REAL, rage_gain REAL, conditions TEXT,
 hit_interval REAL NOT NULL DEFAULT 0, secondary_target_ratio REAL NOT NULL DEFAULT 1, blocks_basic_attack INTEGER NOT NULL DEFAULT 0,
 affected_by_atk_speed INTEGER NOT NULL DEFAULT 0, initial_cooldown REAL NOT NULL DEFAULT 0, priority INTEGER NOT NULL DEFAULT 0,
 trigger_event TEXT NOT NULL DEFAULT 'always', internal_cd REAL NOT NULL DEFAULT 0, direct_damage INTEGER NOT NULL DEFAULT 1, notes TEXT,
 PRIMARY KEY(hero_id, skill_id)
);
CREATE TABLE IF NOT EXISTS hero_damage_profiles (
 hero_id TEXT NOT NULL REFERENCES heroes(hero_id), scenario_id TEXT NOT NULL REFERENCES scenarios(scenario_id),
 basic_share REAL NOT NULL CHECK(basic_share >= 0), skill_share REAL NOT NULL CHECK(skill_share >= 0), ultimate_share REAL NOT NULL CHECK(ultimate_share >= 0),
 expected_targets_basic REAL NOT NULL CHECK(expected_targets_basic >= 0), expected_targets_skill REAL NOT NULL CHECK(expected_targets_skill >= 0),
 expected_targets_ult REAL NOT NULL CHECK(expected_targets_ult >= 0), ult_uptime_base REAL NOT NULL CHECK(ult_uptime_base BETWEEN 0 AND 1),
 PRIMARY KEY(hero_id, scenario_id), CHECK(basic_share + skill_share + ultimate_share > 0)
);
CREATE TABLE IF NOT EXISTS sets (
 set_id TEXT PRIMARY KEY, set_name TEXT NOT NULL, required_pieces INTEGER NOT NULL CHECK(required_pieces > 0),
 slot_group TEXT, output_set INTEGER NOT NULL DEFAULT 0 CHECK(output_set IN (0,1))
);
CREATE TABLE IF NOT EXISTS equipment (
 item_id TEXT PRIMARY KEY, slot TEXT NOT NULL CHECK(slot IN ('weapon','armor','bracelet','necklace','ring')),
 set_id TEXT NOT NULL REFERENCES sets(set_id), tier TEXT, level INTEGER, locked INTEGER NOT NULL DEFAULT 0 CHECK(locked IN (0,1)),
...
CREATE TABLE IF NOT EXISTS set_effects (
 ...
 value REAL NOT NULL, applies_to TEXT NOT NULL, trigger TEXT NOT NULL, duration REAL, max_stacks INTEGER NOT NULL DEFAULT 1,
 stack_rule TEXT NOT NULL DEFAULT 'add', proc_chance REAL NOT NULL DEFAULT 1 CHECK(proc_chance BETWEEN 0 AND 1), internal_cd REAL NOT NULL DEFAULT 0,
 condition TEXT, approximate INTEGER NOT NULL DEFAULT 0 CHECK(approximate IN (0,1)), requires_dot INTEGER NOT NULL DEFAULT 0 CHECK(requires_dot IN (0,1)),
 enabled_in_v1_1 INTEGER NOT NULL DEFAULT 1 CHECK(enabled_in_v1_1 IN (0,1)), PRIMARY KEY(set_id, effect_id)
);
CREATE TABLE IF NOT EXISTS scenarios (
 scenario_id TEXT PRIMARY KEY, scenario_name TEXT NOT NULL, duration REAL NOT NULL CHECK(duration > 0), target_mode TEXT NOT NULL CHECK(target_mode IN ('single','aoe')),
 target_count INTEGER NOT NULL CHECK(target_count > 0), target_def REAL NOT NULL DEFAULT 0, target_mres REAL, spawn_pattern TEXT NOT NULL,
 kill_rate_hint REAL NOT NULL DEFAULT 0, target_hp REAL, weight_primary REAL NOT NULL DEFAULT 1, weight_secondary REAL NOT NULL DEFAULT 1
);
CREATE TABLE IF NOT EXISTS game_rules (
 rule_key TEXT PRIMARY KEY, rule_value TEXT NOT NULL, value_type TEXT NOT NULL CHECK(value_type IN ('number','boolean','string','json')), description TEXT
);
CREATE TABLE IF NOT EXISTS equipment_recognition (
 item_id TEXT PRIMARY KEY REFERENCES equipment(item_id) ON DELETE CASCADE,
 profile TEXT NOT NULL CHECK(profile IN ('exclusive','general')),
 fully_unlocked INTEGER NOT NULL CHECK(fully_unlocked IN (0,1)),
 quality_text TEXT, slot_text TEXT, primary_text TEXT,
...
class EquipmentDatabase:
    ...
    def initialize(self) -> None:
        ...
        defaults = [
            ...
            ("attack_pct_mode", '"base_plus_flat"', "string", "攻击百分比合成模式"),
            ("defense_formula", '"linear"', "string", "防御减伤公式"),
            ("defense_constant", "100.0", "number", "线性防御公式常数"),
            ("attack_interval_base", "1.0", "number", "默认攻击间隔"),
        ]
    ...
    def _migrate_v22_operational_columns(self) -> None:
        columns = {
            "skills": {
                ...
                "blocks_basic_attack": "INTEGER NOT NULL DEFAULT 0", "affected_by_atk_speed": "INTEGER NOT NULL DEFAULT 0",
                "initial_cooldown": "REAL NOT NULL DEFAULT 0", "priority": "INTEGER NOT NULL DEFAULT 0",
                "trigger_event": "TEXT NOT NULL DEFAULT 'always'", "internal_cd": "REAL NOT NULL DEFAULT 0",
        ...
        for table, additions in columns.items():
            ...
            for name, declaration in additions.items():
                legacy_name = {
                    "main_stat_name": "主词条", "main_stat_value": "主词条数值",
                    "sub_stat_1_name": "副词条1", "sub_stat_1_value": "副词条1数值",
                    "sub_stat_2_name": "副词条2", "sub_stat_2_value": "副词条2数值",
                    "sub_stat_3_name": "副词条3", "sub_stat_3_value": "副词条3数值",
                    "sub_stat_4_name": "副词条4", "sub_stat_4_value": "副词条4数值",
                }.get(name)
                if name not in existing and legacy_name in existing:
                    self.connection.execute(f'ALTER TABLE {table} RENAME COLUMN "{legacy_name}" TO {name}')
                    existing.add(name)
        ...
        for row in self.connection.execute("SELECT item_id, raw_result FROM equipment_recognition").fetchall():
            ...
            self.connection.execute(
                """UPDATE equipment_recognition SET
                   main_stat_name=?, main_stat_value=?, sub_stat_1_name=?, sub_stat_1_value=?,
                   sub_stat_2_name=?, sub_stat_2_value=?, sub_stat_3_name=?, sub_stat_3_value=?,
    ...
    def find_upgrade_match(self, record: dict) -> str | None:
        ...
        for row in rows:
            if row["item_id"] == record.get("item_id"):
                continue
            try:
                previous = json.loads(row["raw_result"])
            except (TypeError, json.JSONDecodeError):
                continue
            if is_upgrade_of(previous, record):
                return row["item_id"]
        return None
        ...
        self.connection.execute(
            """INSERT INTO equipment_recognition(
               ...
               ON CONFLICT(item_id) DO UPDATE SET
                 ...
                 set_name_text=excluded.set_name_text, raw_result=excluded.raw_result,
                 source_screenshot=excluded.source_screenshot, recognized_at=excluded.recognized_at""",
            recognition,
        )
        self.connection.commit()
        self._learn_recognized_stats(stored_record, item_id=item[0])
        return {
            "item_id": item[0],
            "detected_item_id": detected_item_id,
            "matched_upgrade": matched_item_id is not None,
            "matched_previous_item_id": matched_item_id,
        }
    ...
    def seed_full_fixture(self) -> None:
        ...
        skills = [
            ("H1", "BASIC_01", "普攻", "basic", "ATK", 1.0, 1, 0.0, "1", 1.0, 1, None, 0.0, 0.0, 20.0, 0, 1, 0.0, 0, "always", 0.0, 1, ""),
            ("H1", "SKILL_01", "技能", "skill", "ATK", 1.5, 2, 0.1, "1", 1.0, 1, 8.0, 0.5, 0.0, 10.0, 1, 0, 0.0, 1, "after_basic", 0.0, 1, ""),
            ("H1", "ULT_01", "终结技", "ultimate", "ATK", 3.0, 1, 0.0, "all", 0.8, 1, None, 0.0, 100.0, 0.0, 1, 1, 0.0, 2, "always", 0.0, 1, ""),
    ...
    def _one(self, sql: str, params: tuple = ()) -> sqlite3.Row:
        ...
        if row is None:
            raise LookupError(f"No database record for query: {sql}")
        return row
    ...
    def load_hero(self, hero_id: str):
        ...
        r = self._one("SELECT * FROM heroes WHERE hero_id=?", (hero_id,))
        return Hero(r["hero_id"], r["hero_name"], r["atk_base"], r["crit_rate_base"], r["crit_dmg_base"], r["atk_speed_base"], r["atk_interval_base"], r["rage_start"], r["rage_max"], DamageType(r["damage_type"]), MainOutput(r["main_output"]))

    def load_profile(self, hero_id: str, scenario_id: str) -> DamageProfile:
        r = self._one("SELECT * FROM hero_damage_profiles WHERE hero_id=? AND scenario_id=?", (hero_id, scenario_id))
        return DamageProfile(*(r[k] for k in ("hero_id", "scenario_id", "basic_share", "skill_share", "ultimate_share", "expected_targets_basic", "expected_targets_skill", "expected_targets_ult", "ult_uptime_base")))

    def load_skills(self, hero_id: str) -> list[Skill]:
        rows = self.connection.execute("SELECT * FROM skills WHERE hero_id=? ORDER BY priority, skill_id", (hero_id,)).fetchall()
        return [Skill(r["hero_id"], r["skill_id"], r["skill_name"], SourceType(r["source_type"]), r["scaling_stat"], r["coefficient"], r["hit_count"], r["hit_interval"], r["target_cap"], r["secondary_target_ratio"], bool(r["can_crit"]), r["cooldown"], r["action_time"] or 0.0, bool(r["blocks_basic_attack"]), bool(r["affected_by_atk_speed"]), r["rage_cost"] or 0.0, r["rage_gain"] or 0.0, r["initial_cooldown"] or 0.0, r["priority"], r["trigger_event"], r["internal_cd"] or 0.0, bool(r["direct_damage"]), r["notes"]) for r in rows]

    def load_scenario(self, scenario_id: str):
        from equipment_models import Scenario
        r = self._one("SELECT * FROM scenarios WHERE scenario_id=?", (scenario_id,))
        return Scenario(*(r[k] for k in ("scenario_id", "scenario_name", "duration", "target_mode", "target_count", "target_def", "target_mres", "spawn_pattern", "kill_rate_hint", "target_hp", "weight_primary", "weight_secondary")))

    def load_sets(self) -> dict[str, SetDefinition]:
        return {r["set_id"]: SetDefinition(r["set_id"], r["set_name"], r["required_pieces"], r["slot_group"], bool(r["output_set"])) for r in self.connection.execute("SELECT * FROM sets")}

    def load_set_effects(self) -> list[SetEffect]:
        supported = ",".join(f"'{effect.value}'" for effect in EffectType)
        return [SetEffect(r["set_id"], r["effect_id"], EffectType(r["effect_type"]), r["value"], r["applies_to"], r["trigger"], r["duration"], r["max_stacks"], r["stack_rule"], r["proc_chance"], r["internal_cd"], r["condition"], bool(r["approximate"]), bool(r["requires_dot"]), bool(r["enabled_in_v1_1"])) for r in self.connection.execute(f"SELECT * FROM set_effects WHERE enabled_in_v1_1=1 AND effect_type IN ({supported})")]

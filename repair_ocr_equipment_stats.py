...
def repair_recognized_stats(database: EquipmentDatabase, *, dry_run: bool = False) -> dict[str, int]:
    rows = database.connection.execute("SELECT item_id, raw_result FROM equipment_recognition ORDER BY recognized_at").fetchall()
    repaired = 0
    skipped = 0
    written_stats = 0
    database.connection.execute("SAVEPOINT repair_ocr_stats")
    try:
        for row in rows:
            try:
            ...
            database.connection.executemany("""INSERT INTO equipment_stats(item_id, stat_index, stat_source, stat_type, stat_value, unlock_level, is_unlocked) VALUES (?, ?, ?, ?, ?, ?, ?)""", stats)
            repaired += 1
            written_stats += len(stats)
        if dry_run:
            database.connection.execute("ROLLBACK TO repair_ocr_stats")
        database.connection.execute("RELEASE repair_ocr_stats")
        if not dry_run:
...
def main() -> int:
    ...
    print(f"{mode} recognized={result['recognized']} repaired={result['repaired']} skipped={result['skipped']} written_stats={result['written_stats']}")
    return 0

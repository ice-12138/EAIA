...
class SubStatEstimator:
    ...
    def observe(self, *, item_id: str, stat_type: str, roll_grade_id: str, value: float,
        ...
        duplicate = self.connection.execute(
            ...
            (item_id, stat_type, roll_grade_id)).fetchone()
        if duplicate:
            return "duplicate"
        row = self.connection.execute(
            "SELECT * FROM stat_value_ranges WHERE stat_type=? AND roll_grade_id=?",
            (stat_type, roll_grade_id)).fetchone()
        ...
        if row is None:
            ...
            values = (stat_type, roll_grade_id, min(row["observed_min"], value),
                      max(row["observed_max"], value), row["verified_min"], row["verified_max"],
                      row["sample_count"] + 1, row["range_status"], data_source, now)
        self.connection.execute(
            """INSERT INTO stat_value_ranges
               ...
               ON CONFLICT(stat_type,roll_grade_id) DO UPDATE SET observed_min=excluded.observed_min,
                observed_max=excluded.observed_max, sample_count=excluded.sample_count,
                data_source=excluded.data_source, updated_at=excluded.updated_at""", values)
        item_exists = self.connection.execute("SELECT 1 FROM equipment WHERE item_id=?", (item_id,)).fetchone()
        if item_exists:
            self.connection.execute("INSERT INTO sub_stat_observations VALUES (?,?,?,?,?,?)",
    ...
    def _queue(self, item_id, stat_type, grade, value, source, reason):
        self.connection.execute(
            "INSERT INTO ocr_import_queue(item_id,stat_type,roll_grade_id,stat_value,data_source,reason,created_at) VALUES (?,?,?,?,?,?,?)",
    ...
    def estimate(self, stat_type: str, roll_grade_id: str | None = None) -> SubStatEstimate:
        if roll_grade_id:
            rows = self.connection.execute("SELECT * FROM stat_value_ranges WHERE stat_type=? AND roll_grade_id=?", (stat_type, roll_grade_id)).fetchall()
        else:
            rows = self.connection.execute("SELECT * FROM stat_value_ranges WHERE stat_type=?", (stat_type,)).fetchall()
        verified = [r for r in rows if r["verified_min"] is not None and r["verified_max"] is not None]
        if verified:
            low, high, source = min(r["verified_min"] for r in verified), max(r["verified_max"] for r in verified), "verified"
        elif rows and all(r["sample_count"] >= self.min_samples for r in rows):
            low, high, source = min(r["observed_min"] for r in rows), max(r["observed_max"] for r in rows), "observed"
        else:

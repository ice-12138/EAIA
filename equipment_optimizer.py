"""V0.1 static-panel and simplified-damage equipment optimizer."""
...
from equipment_models import BattleConfig, BuildResult, EquipmentItem, Panel, SimulationResult, SourceType, StatType
...
class EquipmentOptimizer:
    ...
    def calculate_panel(self, hero_id: str, items: tuple[EquipmentItem, ...]) -> tuple[Panel, tuple[str, ...]]:
        ...
        for effect in effects:
            effect_values[effect.effect_type] += effect.value
        ...
        panel = Panel(
            ...
            rage_regen=stats[StatType.RAGE_REGEN.value] + effect_values["RAGE_REGEN"],
        )
    ...
    def _score(self, hero_id: str, scenario_id: str, items: tuple[EquipmentItem, ...]) -> BuildResult:
        ...
        general = sum(e.value for e in effects if e.set_id in active_sets and e.effect_type == "DAMAGE_PCT")
        source_bonus = {source: general for source in ("basic", "skill", "ultimate")}
        for effect in effects:
            if effect.set_id in active_sets:
                key = {"BASIC_DMG": "basic", "SKILL_DMG": "skill", "ULT_DMG": "ultimate"}.get(effect.effect_type)
                if key: source_bonus[key] += effect.value
        ...
        source_damage = {source: panel.atk * share * target_values[source] * crit_factor * (1 + source_bonus[source]) * defense_factor
    ...
    def search(self, hero_id: str, mode: str = "single", enemy_count: int = 1, top_k: int = 10) -> list[SimulationResult | BuildResult]:
        # Keep the V0.1 call shape search(hero_id, scenario_id, top_k=...) readable
        # while making V1.1 mode/enemy_count the primary interface.
        ...
        for left_items in left:
            for right_items in right:
                ...
                if len(heap) < top_k: heapq.heappush(heap, candidate)
                elif candidate.score > heap[0].score: heapq.heapreplace(heap, candidate)
